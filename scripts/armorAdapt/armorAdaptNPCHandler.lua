...
adaptItems = root.assetJson(string.format("/armorAdapt/armorAdaptList_%s.json:supportedItems",self.npcSpecies)) or {}
		...
		for k,slot in ipairs(slotTable) do
			...
			if equippedArmorTimer <= 0 then
				for _,armorItem in ipairs(adaptItems) do
					if root.itemDescriptorsMatch(adaptNpcArmor[k], armorItem) then
						sb.logInfo("first descriptor match done.")
						...
						storageArmorTable = armorAdapt.generateNpcArmorTable()
						--sb.logInfo("Is the equipped armor the same? %s", changed)
					else
						if (played[3] == 0) and (adaptDebug.showSupportedItem == true) then
							sb.logInfo("[Armor Adapt][NPC Handler]: No supported armor found, continuing search")
							played[3] = 1
						end
					end
