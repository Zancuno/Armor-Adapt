	...
	if changed == false then
		for v,armorSpecies in ipairs(adaptSpecies.supportedSpecies) do
			if self.playerSpecies == armorSpecies then
				if played[8] == 0 and (adaptDebug.showPlayerSpecies == true) then
				...
				bodyTable = armorAdapt.getSpeciesBodyTable(self.playerSpecies) or bodyTable
					...
					bodyAlt = bodyTable[4]
					bodyOther = bodyTable[5]
				if played[6] == 0 and (adaptDebug.showPlayerBodyType == true) then
				...
				if played[5] == 0 and (adaptDebug.showPlayerSpecies == true) then
		...
		if played[1] == 0 and (adaptDebug.showPlayerArmor == true) then
		...
		for k,slot in ipairs(slotTable) do
			...
			if equippedArmorTimer <= 0 then
				...
				adaptArmorPlayerItem = copy(adaptPlayerArmor[k])
					if played[2] == 0 and (adaptDebug.showPlayerSupportedItem == true) then
				...
				elseif armorType == 5 or armorType == 6 then
					...
					if played[4] == 0 and (adaptDebug.showPlayerBuildInfo == true) then
					...
					if adaptDebug.showPlayerBuildCompletion == true then
				...
				elseif root.itemConfig(adaptPlayerArmor[k]).parameters.itemTags ~= nil and root.itemConfig(adaptPlayerArmor[k]).parameters.itemTags[2] == self.playerSpecies and root.itemConfig(adaptPlayerArmor[k]).parameters.itemTags[3] == bodyType then
					storageArmorTable[k] = adaptPlayerArmor[k]
				end
			end
